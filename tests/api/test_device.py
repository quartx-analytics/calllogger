...
def test_success(requests_mock, mock_running):
    ...
    mocked_request = requests_mock.post(info.linking_url, status_code=201, json=expected_resp)
    resp = info.link_device(mock_identifier)
...
def test_retry(requests_mock, mock_running, disable_sleep, timeout, mock_settings):
    ...
    mocked_request = requests_mock.post(info.linking_url, response_list=[
        {"status_code": 204},
        {"status_code": 201, "json": expected_resp},
    ])

    info.link_device(mock_identifier)
...
def test_unexpected_status(requests_mock, mock_running):
    mocked_request = requests_mock.post(info.linking_url, status_code=202)
    resp = info.link_device(mock_identifier)
...
def test_errored(requests_mock, mock_running, disable_sleep):
    mocked_request = requests_mock.post(info.linking_url, exc=RuntimeError)
